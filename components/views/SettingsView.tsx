...
import { Goal, Account, AutomationRule, TransactionType, Category } from '../../types';
...
const SettingsView: React.FC = () => {
    // FIX: Added `categories` to the destructuring from `useAppContext` to resolve reference error.
    const { goals, handleDeleteGoal, accounts, handleDeleteAccount, automationRules, handleDeleteAutomationRule, categories } = useAppContext();
    ...
    const [isRuleModalOpen, setIsRuleModalOpen] = useState(false); const [selectedRule, setSelectedRule] = useState<AutomationRule | undefined>(undefined);
    ...
    return (
        <>
            <div className="space-y-8 max-w-6xl mx-auto">
                ...
                <div className="bg-slate-800 p-6 rounded-xl shadow-lg"><h2 className="text-2xl font-semibold mb-4 flex items-center gap-2"><SparklesIcon className="w-6 h-6 text-violet-400" />Reglas de Automatización</h2><p className="text-gray-400 mb-4 text-sm">Ahorra tiempo enseñando a la app cómo categorizar tus importaciones. Las reglas se aplican automáticamente al subir un archivo.</p><button onClick={() => { setSelectedRule(undefined); setIsRuleModalOpen(true); }} className="w-full flex items-center justify-center space-x-2 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold py-2 px-4 rounded-lg mb-4"><PlusCircleIcon className="w-5 h-5" /><span>Crear Nueva Regla</span></button><div className="space-y-2">{automationRules.map(rule => (<div key={rule.id} className="flex justify-between items-center bg-slate-700 p-3 rounded-md"><div><p className="font-semibold text-white">Si la descripción contiene: <span className="font-bold text-violet-300">"{rule.keyword}"</span></p><p className="text-sm text-gray-400">Asignar a la categoría de {rule.type === TransactionType.EXPENSE ? 'gasto' : 'ingreso'}: <span className="font-semibold text-gray-200">{categories.find(c => c.id === rule.categoryId)?.name || 'N/A'}</span></p></div><div className="flex items-center space-x-3"><button onClick={() => { setSelectedRule(rule); setIsRuleModalOpen(true); }} className="text-gray-400 hover:text-violet-400"><PencilIcon className="w-4 h-4" /></button><button onClick={() => onDeleteRule(rule)} className="text-gray-400 hover:text-rose-500"><TrashIcon className="w-4 h-4" /></button></div></div>))}</div></div>
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
